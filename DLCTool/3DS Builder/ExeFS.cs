...
namespace CTR
{
    public class ExeFS
    {
        ...
        internal static bool get(byte[] data, string outPath)
        {
            try
            {
                if (!Directory.Exists(outPath)) Directory.CreateDirectory(outPath);
        ...
        internal static bool validate(byte[] data)
        {
            try
            {
                for (int i = 0; i < 10; i++)
                {
                    // Get File Name String; if exists we have a file to extract.
                    string fileName = Encoding.ASCII.GetString(data, 0x10 * i, 8).TrimEnd((char)0);

                    if (fileName.Length > 0)
                    {
                        uint file_ofs = BitConverter.ToUInt32(data, 0x10 * i + 0x8) + 0x200;
                        uint file_len = BitConverter.ToUInt32(data, 0x10 * i + 0xC);
                        byte[] filehash = new byte[0x20];
                        Array.Copy(data, 0x1E0 - (0x20 * i), filehash, 0, 0x20);
                        byte[] file = new byte[file_len];
                        Array.Copy(data, file_ofs, file, 0, file_len);
                        byte[] calchash;
                        using (SHA256Managed sha = new SHA256Managed())
                            calchash = sha.ComputeHash(file);
                        if (!calchash.SequenceEqual(filehash))
                            return false;
                    }
                }
                return true;
