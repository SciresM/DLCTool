...
namespace CTR
{
    public class RomFS
    {
        ...
        internal static void ExtractRomFS(string ROOT_DIR, string infile, RichTextBox RTB_Progress = null, ProgressBar PB_Show = null, Label ProgressLabel = null)
        {
            using (BinaryReader br = new BinaryReader(File.OpenRead(infile)))
            {
                br.BaseStream.Seek(0x1000, SeekOrigin.Begin);
                br.ReadBytes(0xC);
                uint DIR_TABLE_OFS;
                uint FILE_TABLE_OFS;
                uint DATA_OFS;
                DIR_TABLE_OFS = 0x1000 + br.ReadUInt32();
                uint DirTableLen = br.ReadUInt32();
                br.ReadBytes(0x8);
                FILE_TABLE_OFS = 0x1000 + br.ReadUInt32();
                uint FileTableLen = br.ReadUInt32();
                DATA_OFS = 0x1000 + br.ReadUInt32();
                List<Romfs_DirEntry> Directories = new List<Romfs_DirEntry>();
                List<Romfs_FileEntry> Files = new List<Romfs_FileEntry>();
                br.BaseStream.Seek(DIR_TABLE_OFS, SeekOrigin.Begin);
                while (br.BaseStream.Position < DIR_TABLE_OFS + DirTableLen) //Add all Directories
                {

                    Romfs_DirEntry Dir = new Romfs_DirEntry();
                    Dir.Offset = (uint)(br.BaseStream.Position - DIR_TABLE_OFS);
                    Dir.ParentOffset = br.ReadUInt32();
                    string Parent = (Dir.Offset == 0) ? "" : Directories[GetRomfsDirEntry(Directories, Dir.ParentOffset)].FullName;
                    Dir.SiblingOffset = br.ReadUInt32();
                    Dir.ChildOffset = br.ReadUInt32();
                    Dir.FileOffset = br.ReadUInt32();
                    Dir.HashKeyPointer = br.ReadUInt32();
                    int NameLen = br.ReadInt32();
                    if (NameLen > 0)
                    {
                        Dir.Name = Encoding.Unicode.GetString(br.ReadBytes(NameLen));
                        while (br.BaseStream.Position % 4 != 0)
                        {
                            br.ReadByte();
                        }
                        Dir.FullName = Parent + Path.DirectorySeparatorChar + Dir.Name;
                    }
                    else
                    {
                        Dir.Name = "";
                ...
                br.BaseStream.Seek(FILE_TABLE_OFS, SeekOrigin.Begin);
                while (br.BaseStream.Position < FILE_TABLE_OFS + FileTableLen) // Add all files
                {
                    Romfs_FileEntry file = new Romfs_FileEntry();
                    file.ParentDirOffset = br.ReadUInt32();
                    string Parent = Directories[GetRomfsDirEntry(Directories, file.ParentDirOffset)].FullName;
                    file.SiblingOffset = br.ReadUInt32();
                    file.DataOffset = br.ReadUInt64();
                    file.DataSize = br.ReadUInt64();
                    file.HashKeyPointer = br.ReadUInt32();
                    int NameLen = br.ReadInt32();
                    file.Name = Encoding.Unicode.GetString(br.ReadBytes(NameLen));
                    while (br.BaseStream.Position % 4 != 0)
                    {
                        br.ReadByte();
                    }
                    file.FullName = Parent + Path.DirectorySeparatorChar + file.Name;
                    Files.Add(file);
                }
                CTR_ROM.updateTB(RTB_Progress, "Parsed " + Directories.Count + " Directories and " + Files.Count + " Files.");
                CTR_ROM.updateTB(RTB_Progress, "Extracting Files...");
                PB_Show.Invoke((Action)(() =>
                {
                    PB_Show.Minimum = 0;
                    PB_Show.Maximum = Files.Count;
                    PB_Show.Value = 0;
                    PB_Show.Step = 1;
                }));
                if (ProgressLabel.InvokeRequired)
                    ProgressLabel.Invoke((MethodInvoker)delegate { ProgressLabel.Visible = true; });
                else { ProgressLabel.Visible = true; }
                int i = 0;
                foreach (Romfs_FileEntry f in Files)
                {
                    string dir = ROOT_DIR + Path.GetDirectoryName(f.FullName) + Path.DirectorySeparatorChar;
                    if (!Directory.Exists(dir))
                    {
                        Directory.CreateDirectory(dir);
                    }
                    br.BaseStream.Seek(DATA_OFS + (long)f.DataOffset, SeekOrigin.Begin);
                    uint BUFFER_LEN = 0x400000;
                    using (FileStream fs = new FileStream(ROOT_DIR + f.FullName, FileMode.OpenOrCreate))
                    {
                        ulong read = 0;
                        byte[] buf;
                        while (read < f.DataSize)
                        {
                            if (f.DataSize - read > BUFFER_LEN)
                            {
                                buf = new byte[BUFFER_LEN];
                            }
                            else
                            {
                                buf = new byte[f.DataSize - read];
                            }
                            br.BaseStream.Read(buf, 0, buf.Length);
                            fs.Write(buf, 0, (int)buf.Length);
                            read += (ulong)buf.Length;
                        }
                    }
                    i++;
                    PB_Show.Invoke((Action)(() => PB_Show.PerformStep()));
                    if (ProgressLabel.InvokeRequired)
                        ProgressLabel.Invoke((MethodInvoker)delegate { ProgressLabel.Text = String.Format("{0:P2} - {1}/{2}", ((float)i) / ((float)Files.Count), i, Files.Count); });
                    else { ProgressLabel.Text = String.Format("{0:P2} - {1}/{2}", ((float)i) / ((float)Files.Count), i, Files.Count); }
                }
                if (ProgressLabel.InvokeRequired)
                    ProgressLabel.Invoke((MethodInvoker)delegate { ProgressLabel.Visible = false; });
                else { ProgressLabel.Visible = false; }
            }
        }

        private static int GetRomfsDirEntry(List<Romfs_DirEntry> Directories, uint Offset)
        {
