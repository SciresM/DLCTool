...
namespace CTR
{
    public class CTR_ROM
    {
        ...
        internal static bool buildROM(bool Card2, byte[] LOGO,
            string EXEFS_PATH, string ROMFS_PATH, string EXHEADER_PATH,
            string SERIAL_TEXT, string SAVE_PATH, byte[] MANUAL = null,
            ProgressBar PB_Show = null, RichTextBox TB_Progress = null)
        {
            ...
            var NCCH = setNCCH(EXEFS_PATH, ROMFS_PATH, EXHEADER_PATH, SERIAL_TEXT, LOGO, PB_Show, TB_Progress);
            var NCSD = setNCSD(NCCH, Card2, MANUAL, PB_Show, TB_Progress);
            bool success = writeROM(NCSD, SAVE_PATH, PB_Show, TB_Progress);
            return success;
        }

        internal static bool buildCIA(byte[] LOGO,
            string EXEFS_PATH, string ROMFS_PATH, string EXHEADER_PATH,
            string SERIAL_TEXT, string SAVE_PATH, byte[] MANUAL = null,
            ProgressBar PB_Show = null, RichTextBox TB_Progress = null)
        {
            ...
            bool success = writeNoCryptoNCCH(NCCH, "Content.ncch", PB_Show, TB_Progress);
            string command = string.Format(" -f cia -rsf rom.rsf -o temp.cia -ckeyid 0 -minor 0 -micro 0 -DSaveSize=2097152", SAVE_PATH);
            command += " -i Content.ncch:0x0000";
            if (MANUAL != null)
            {
                File.WriteAllBytes("Manual.ncch", MANUAL);
                command += " -i Manual.ncch:0x0001";
            }
            updateTB(TB_Progress, "Building CIA using Makerom...");
            Process create = new Process();
            create.StartInfo.FileName = "makerom.exe";
            create.StartInfo.WindowStyle = ProcessWindowStyle.Hidden;
            create.StartInfo.Arguments = command;
            create.Start();
            create.WaitForExit();
            updateTB(TB_Progress, "Cleaning up temporary files...");
            updateTB(TB_Progress, "Done!");
            if (File.Exists(SAVE_PATH))
                File.Delete(SAVE_PATH);
            File.Move("temp.cia", SAVE_PATH);
            File.Delete("Manual.ncch");
            File.Delete("Content.ncch");
            return success;
        }

        // Sub methods that drive the operation
        internal static NCCH setNCCH(string EXEFS_PATH, string ROMFS_PATH, string EXHEADER_PATH, string TB_Serial, byte[] LOGO,
            ProgressBar PB_Show = null, RichTextBox TB_Progress = null)
        {
            PB_Show = PB_Show ?? new ProgressBar();
            TB_Progress = TB_Progress ?? new RichTextBox();
        ...
        internal static NCSD setNCSD(NCCH Content, bool Card2, byte[] Manual = null,
            ProgressBar PB_Show = null, RichTextBox TB_Progress = null)
        {
            PB_Show = PB_Show ?? new ProgressBar();
            TB_Progress = TB_Progress ?? new RichTextBox();
            ...
            if (Manual != null)
                Rom.NCCH_Array.Add(new NCCH(Manual));
        ...
        internal static bool writeNoCryptoNCCH(NCCH Content, string SAVE_PATH, 
            ProgressBar PB_Show = null, RichTextBox TB_Progress = null)
        {
            //Make it no Crypto
            Content.header.Flags[7] |= 4; // FixedCrypto = 1, NoMountRomfs = 2; NoCrypto=4;
            Content.header.BuildHeader();
            PB_Show = PB_Show ?? new ProgressBar();
            TB_Progress = TB_Progress ?? new RichTextBox();
            using (FileStream OutFileStream = new FileStream(SAVE_PATH, FileMode.Create))
            {
                updateTB(TB_Progress, "Writing Content NCCH...");
                OutFileStream.Write(Content.header.Data, 0, Content.header.Data.Length); //Write NCCH header
                updateTB(TB_Progress, "Writing Exheader...");
                byte[] Exheader = new byte[Content.exheader.Data.Length + Content.exheader.AccessDescriptor.Length];
                Array.Copy(Content.exheader.Data, Exheader, Content.exheader.Data.Length);
                Array.Copy(Content.exheader.AccessDescriptor, 0, Exheader, Content.exheader.Data.Length, Content.exheader.AccessDescriptor.Length);
                OutFileStream.Write(Exheader, 0, Exheader.Length); // Write Exheader
                updateTB(TB_Progress, "Writing Exefs...");
                OutFileStream.Seek(Content.header.ExefsOffset * MEDIA_UNIT_SIZE, SeekOrigin.Begin);
                byte[] OutExefs = new byte[Content.exefs.Data.Length];
                OutFileStream.Write(Content.exefs.Data, 0, Content.exefs.Data.Length);
                updateTB(TB_Progress, "Writing Romfs...");
                OutFileStream.Seek(Content.header.RomfsOffset * MEDIA_UNIT_SIZE, SeekOrigin.Begin);
                using (FileStream InFileStream = new FileStream(Content.romfs.FileName, FileMode.Open, FileAccess.Read))
                {
                    uint BUFFER_SIZE = 0;
                    ulong RomfsLen = Content.header.RomfsSize * MEDIA_UNIT_SIZE;
                ...
                OutFileStream.Seek(Content.header.LogoOffset * MEDIA_UNIT_SIZE, SeekOrigin.Begin);
                OutFileStream.Write(Content.logo, 0, Content.logo.Length);
            }
            ...
            ProgressBar PB_Show = null, RichTextBox TB_Progress = null)
        {
            ...
            using (FileStream OutFileStream = new FileStream(SAVE_PATH, FileMode.Create))
            {
                ...
                if (Rom.NCCH_Array.Count > 1 && Rom.NCCH_Array[1].isManual)
                {
                    updateTB(TB_Progress, "Writing Manual...");
                    OutFileStream.Seek(MEDIA_UNIT_SIZE * Rom.header.OffsetSizeTable[1].Offset, SeekOrigin.Begin);
                    OutFileStream.Write(Rom.NCCH_Array[1].Manual, 0, Rom.NCCH_Array[1].Manual.Length);
                }
